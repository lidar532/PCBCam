...
from tkinter import filedialog, messagebox, Toplevel, Scale, Spinbox, Entry
import json
import queue
import os
...
class AppGUI(tk.Tk):
    ...
    def _create_widgets(self):
        ...
        self.tree.heading('index', text='#'); self.tree.heading('x', text='X-Coord'); self.tree.heading('y', text='Y-Coord')
        self.tree.heading('shape', text='Shape'); self.tree.heading('color', text='Color'); self.tree.heading('size', text='Size')
        self.tree.heading('desc', text='Description')
        self.tree.column('index', width=40, anchor=tk.CENTER); self.tree.column('x', width=80, anchor=tk.CENTER)
        self.tree.column('y', width=80, anchor=tk.CENTER); self.tree.column('shape', width=80, anchor=tk.W)
        self.tree.column('color', width=80, anchor=tk.W); self.tree.column('size', width=60, anchor=tk.CENTER)
    ...
    def _create_menus(self):
        ...
        file_menu.add_command(label="Load Markers...", command=self._load_file)
        file_menu.add_command(label="Save", command=self._save_current_file)
        file_menu.add_command(label="Save Markers As...", command=self._save_as_file)
        file_menu.add_separator(); file_menu.add_command(label="Exit", command=self._on_exit)
        camera_menu = tk.Menu(self.menubar, tearoff=0); self.menubar.add_cascade(label="Camera", menu=camera_menu)
        resolution_menu = tk.Menu(camera_menu, tearoff=0); camera_menu.add_cascade(label="Set Resolution", menu=resolution_menu)
        resolutions = [("640x480",(640,480)),("800x600",(800,600)),("1280x720",(1280,720)),("1280x960",(1280,960)),("1600x1200",(1600,1200)),("1920x1080",(1920,1080)),("2048x1536",(2048,1536)),("2592x1944",(2592,1944))]
        for label, (w, h) in resolutions: resolution_menu.add_command(label=label, command=lambda w=w, h=h: self._set_resolution(w, h))
    ...
    def _refresh_marker_table(self):
        ...
        for i, marker in enumerate(self.markers):
            pos, shape = marker['pos'], marker['shape']
            color_name = bgr_to_color_name.get(tuple(marker['color']), "Custom")
            size, desc = marker['size'], marker.get('desc', '')
            self.tree.insert('', tk.END, values=(i + 1, pos[0], pos[1], shape, color_name, f"{size}px", desc))
    ...
    def _check_for_updates(self):
        ...
        try:
            while True:
                ...
                if command in ['sync_markers', 'update_marker']: needs_refresh = True
                if command == 'sync_markers': self.markers = value
                elif command == 'update_marker':
                    index, marker_data = value
                    if 0 <= index < len(self.markers): self.markers[index] = marker_data
                elif command == 'resolution_update': self.current_resolution = value
                elif command == 'confirm_delete_marker':
                    index, marker_data = value; self._confirm_delete(index, marker_data)
                elif command == 'show_description_dialog_for_marker':
                    marker_index = value
                    self._open_description_dialog_event(marker_index=marker_index)
    ...
    def _restart_camera(self):
        print("GUI: Sending restart command to camera process.")
        self.command_queue.put(('restart_camera', None))

    def _confirm_delete(self, index, marker_data):
        marker_num = index + 1
        desc = marker_data.get('desc', ''); details = f"Marker #{marker_num} at {marker_data['pos']}"
        if desc: details += f"\nDescription: {desc}"
        self.attributes('-topmost', True)
        user_response = messagebox.askyesno("Confirm Delete", f"Are you sure you want to delete this marker?\n\n{details}")
    ...
    def _open_description_dialog_event(self, event=None, marker_index=None):
        if not self.markers: messagebox.showinfo("Info", "Please add a marker first."); return
        if marker_index is None: marker_index = len(self.markers) - 1
        DescriptionDialog(self, self.markers, self.colors, self._update_marker_from_dialog, starting_index=marker_index)
        
    def _update_marker_from_dialog(self, index, updated_marker_data):
        if 0 <= index < len(self.markers):
            self.markers[index] = updated_marker_data
            self._refresh_marker_table()
            self.command_queue.put(('update_marker', (index, self.markers[index])))
    ...
    def _new_file(self):
        if messagebox.askyesno("Confirm New", "Clear all current markers?"):
            self.current_filepath = None; self.command_queue.put(('clear_markers', None))
    def _load_file(self):
        ...
        try:
            ...
            self.command_queue.put(('load_file', data))
        except Exception as e: messagebox.showerror("Error", f"Failed to load file.\n{e}")
    def _save_current_file(self):
        if self.current_filepath:
            data_to_save = {"resolution": self.current_resolution, "markers": self.markers}
            try:
...
# --- CHANGED: This entire class is updated ---
class DescriptionDialog(Toplevel):
    def __init__(self, parent, markers, colors, callback, starting_index=0):
        super().__init__(parent)
        self.markers = markers
        self.colors = colors
        self.bgr_to_color_name = {v: k for k, v in self.colors.items()}
        self.callback = callback

        self.title("Edit Marker Properties"); self.geometry("400x300")
        
        # --- Variables ---
        self.marker_num_var = tk.IntVar(); self.x_var = tk.IntVar(); self.y_var = tk.IntVar()
        self.shape_var = tk.StringVar(); self.color_var = tk.StringVar(); self.size_var = tk.IntVar()
        ...
        self.marker_spinbox = Spinbox(main_frame, from_=1, to=len(markers) if markers else 1, textvariable=self.marker_num_var, width=7, command=self._on_marker_selection_change)
        self.marker_spinbox.grid(row=0, column=1, sticky="w", pady=2)
        ttk.Label(main_frame, text="X Coordinate:").grid(row=1, column=0, sticky="w", pady=2)
        self.x_spinbox = Spinbox(main_frame, from_=0, to=9999, textvariable=self.x_var, width=7)
        self.x_spinbox.grid(row=1, column=1, sticky="w", pady=2)
        ttk.Label(main_frame, text="Y Coordinate:").grid(row=2, column=0, sticky="w", pady=2)
        self.y_spinbox = Spinbox(main_frame, from_=0, to=9999, textvariable=self.y_var, width=7)
        self.y_spinbox.grid(row=2, column=1, sticky="w", pady=2)
        ttk.Label(main_frame, text="Shape:").grid(row=3, column=0, sticky="w", pady=2)
        self.shape_combo = ttk.Combobox(main_frame, textvariable=self.shape_var, values=['Cross', 'Circle', 'Square'], state='readonly', width=10)
        self.shape_combo.grid(row=3, column=1, sticky="w", pady=2)
        ttk.Label(main_frame, text="Color:").grid(row=4, column=0, sticky="w", pady=2)
        self.color_combo = ttk.Combobox(main_frame, textvariable=self.color_var, values=list(self.colors.keys()), state='readonly', width=10)
        self.color_combo.grid(row=4, column=1, sticky="w", pady=2)
        ttk.Label(main_frame, text="Size:").grid(row=5, column=0, sticky="w", pady=2)
        self.size_combo = ttk.Combobox(main_frame, textvariable=self.size_var, values=[9, 15, 25], state='readonly', width=10)
        self.size_combo.grid(row=5, column=1, sticky="w", pady=2)
        ...
        self.desc_entry.grid(row=6, column=1, sticky="ew", pady=2)

        btn_frame = ttk.Frame(main_frame)
        ttk.Button(btn_frame, text="OK", command=self._on_ok).pack(side=tk.LEFT, padx=5)
        ttk.Button(btn_frame, text="Update", command=self._on_update).pack(side=tk.LEFT, padx=5) # ADDED
    ...
    # ADDED: This new method applies changes without closing the dialog
    def _apply_changes(self):
        try:
            marker_index = self.marker_num_var.get() - 1
            updated_marker_data = {
                "pos": (self.x_var.get(), self.y_var.get()),
                "shape": self.shape_var.get(),
                "color": self.colors[self.color_var.get()],
                "size": self.size_var.get(),
                "desc": self.desc_var.get()
            }
            self.callback(marker_index, updated_marker_data)
            return True # Indicate success
        except (tk.TclError, KeyError):
            messagebox.showerror("Invalid Input", "Please ensure all fields are set correctly.")
            return False # Indicate failure

    # ADDED: New command for the "Update" button
    def _on_update(self):
        self._apply_changes()

    def _on_ok(self):
        # CHANGED: OK button now uses the shared apply method
