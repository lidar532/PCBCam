...
class CameraHandler:
    def __init__(self, command_queue, update_queue):
        ...
        self.markers, self.undo_stack, self.redo_stack = [], [], []
    ...
    def _sync_gui_markers(self): self.update_queue.put(('sync_markers', self.markers))
    def _undo_action(self):
        if not self.undo_stack: return
        last_action = self.undo_stack.pop(); self.redo_stack.append(last_action)
        action_type = last_action.get('action_type')
        if action_type == 'add': self.markers.pop()
        elif action_type == 'delete': self.markers.insert(last_action['index'], last_action['data'])
        elif action_type == 'modify': self.markers[last_action['index']] = last_action['old_data']
        self._sync_gui_markers()
    def _redo_action(self):
        if not self.redo_stack: return
        last_action = self.redo_stack.pop(); self.undo_stack.append(last_action)
        action_type = last_action.get('action_type')
        if action_type == 'add': self.markers.append(last_action['data'])
        elif action_type == 'delete': del self.markers[last_action['index']]
        elif action_type == 'modify': self.markers[last_action['index']] = last_action['new_data']
        self._sync_gui_markers()
    
    def _get_camera_name(self):
        if sys.platform == "win32": return self._get_camera_name_windows()
        elif sys.platform.startswith("linux"): return self._get_camera_name_linux()
        else: return f"Camera {self.device_index}"
    def _get_camera_name_windows(self):
        try:
            ...
            pEnumCat = pSysDevEnum.CreateClassEnumerator(DEVICES_CATEGORY_GUID, 0)
        ...
        except Exception as e: print(f"CAM: Could not get camera name using comtypes. Error: {e}")
    ...
    def _get_camera_name_linux(self):
        try:
            ...
            output = result.stdout.strip(); devices = []; current_device_name = None
            for line in output.split('\n'):
                if not line.startswith('\t'): current_device_name = line.strip().split(' (')[0]
                elif line.strip().startswith('/dev/video'): devices.append(current_device_name)
            if self.device_index < len(devices): return devices[self.device_index]
        ...
        try:
            ...
            if command == 'exit': return False
                ...
                if 0 <= index < len(self.markers):
                    ...
                    self.markers[index] = new_marker_data; self._sync_gui_markers()
                ...
                self.markers.clear(); self.undo_stack.clear(); self.redo_stack.clear(); self._sync_gui_markers()
            elif command == 'load_file':
                self.markers = value.get('markers', []); self.undo_stack.clear(); self.redo_stack.clear()
    ...
    def mouse_events(self, event, x, y, flags, param):
        ...
        if event == cv2.EVENT_LBUTTONDOWN:
            ...
            self.markers.append(new_marker); self.undo_stack.append({'action_type': 'add', 'data': new_marker}); self._sync_gui_markers()
    ...
    def find_and_request_delete(self, window_x, window_y):
        ...
        for i, marker in enumerate(self.markers):
            dist_sq = (marker['pos'][0] - target_x)**2 + (marker['pos'][1] - target_y)**2
            if dist_sq < min_dist_sq: min_dist_sq, nearest_index = dist_sq, i
        if nearest_index != -1: self.update_queue.put(('confirm_delete_marker', (nearest_index, self.markers[nearest_index])))
